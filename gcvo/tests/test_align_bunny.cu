...
int main(int argc, char** argv) {
  ...
  // Load cloud and subsample to n points so src and tgt are balanced
  CloudT src_full = read_cloud_xyz(a.input_pcd_file);
  const int N_full = src_full.size();
  const int n = std::min(a.n, N_full);

  pcl::PointCloud<PointT> src_pcl;
  src_pcl.resize(static_cast<size_t>(n));
  for (int i = 0; i < n; ++i)
    src_pcl[static_cast<size_t>(i)] = src_full.points()[static_cast<size_t>(i)];
  CloudT src(src_pcl);
  ...
  // Evaluate both IPs at l_min: the scale the solver converges to, so the
  // before/after comparison is made at the same resolution as the final alignment.
  ...
  std::cout << "==== test_align_bunny (inner product + SE3 error) ====\n";
  std::cout << "n=" << n
  ...
  std::cout<<"T_gt is "<<T_gt.inverse().eval()<<", T_pred is "<<T_out<<"\n";
  Eigen::Matrix4f err_mat = T_gt * T_out;
  std::cout<<"Log of Error mat is "<< (gcvo::liegroup::Log_SE3<float, Eigen::ColMajor>(err_mat)).norm()<<"\n";
  require((gcvo::liegroup::Log_SE3<float, Eigen::ColMajor>(err_mat)).norm() < 1e-5, "the log error of gt_inv * pred is too large");
